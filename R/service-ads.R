ads_beta_service <- R6::R6Class("ecmwfr_ads", inherit = cds_service,
  public = list(
    submit = function() {
      ...
      private$url <- wf_server(id = ct$jobID, service = "ads")

      return(self)
    },
    browse_request = function() {
      url <- "https://ads-beta.atmosphere.copernicus.eu/requests?tab=all"
      utils::browseURL(url)
