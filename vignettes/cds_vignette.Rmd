---
title: "CDS worked example"
author: "Reto Stauffer, Koen Hufkens"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{CDS worked example}
...
[Copernicus.eu](https://www.copernicus.eu/en) provides a set of interesting data sets for research, education, and applied earth sciences on their [Climate Data Store](https://cds-beta.climate.copernicus.eu) (CDS). The R package `ecmwfr` provides a convenient interface to download these data sets.
...
* [Register yourself for CDS services](https://cds-beta.climate.copernicus.eu/login)
* [Accpet the Terms and Conditions](https://cds-beta.climate.copernicus.eu/profile)

To retrieve data `ecmwfr` use `wf_set_key()`to add the Personal Access Token to your local keyring (requires the _R_ package [`keyring`](https://CRAN.R-project.org/package=keyring)).

## The Request Syntax

CDS data retrievals are based on a `list` object which specifies the data set to be downloaded.  These definitions are called `requests`. A `request` defined the type of the `dataset`, the `variables` to be downloaded, the time period, output `format`, `target` location, a custom `area` extent, and other details.
